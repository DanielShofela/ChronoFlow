                ...
                <motion.div key={`display-${activity.id}`} {...{ initial: { opacity: 0 }, animate: { opacity: 1 } }}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {hiddenNonSelectedHours[activity.id] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                  ...
                  <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-12 gap-2">
                    {Array.from({ length: 24 }, (_, i) => {
                      // N'afficher que les heures sélectionnées si hiddenNonSelectedHours est activé pour cette activité
                      const isSelected = activity.slots.includes(i);
                      const shouldDisplay = !hiddenNonSelectedHours[activity.id] || isSelected;
                      ...
                      return shouldDisplay ? (
                        <button 
                          ...
                          style={{ backgroundColor: isSelected ? activity.color : undefined }}
                        >
                          {i}h
                        </button>
                      ) : (
                        <div key={i} className="px-2 py-1"></div> // Espace vide pour maintenir la grille
                      );
                    })}
                  </div>
                </motion.div>
...
export function SettingsView({ activities, onActivitiesChange, onBack, showVerse, onShowVerseChange, showActivityDetails, onShowActivityDetailsChange }: SettingsViewProps) {
  ...
  // État pour gérer la visibilité des heures non sélectionnées pour chaque activité
  // Par défaut, on masque les heures non sélectionnées pour toutes les activités
  const [hiddenNonSelectedHours, setHiddenNonSelectedHours] = useState<Record<string, boolean>>(() => {
    // Initialiser avec toutes les activités masquées par défaut
    return activities.reduce((acc, activity) => {
      acc[activity.id] = true; // true = masqué
      return acc;
  ...
  const handleToggleSlot = (activityId: string, hour: number) => {
    // Si hour est -1, c'est une demande de basculement de visibilité des heures non sélectionnées
    if (hour === -1) {
      // Basculer l'état de visibilité pour cette activité
      setHiddenNonSelectedHours(prev => ({
        ...prev,
          ...
          <AnimatePresence>
            {activities.map(activity => (
              <ActivityReorderItem
                ...
                hiddenNonSelectedHours={hiddenNonSelectedHours}
                t={t}
              />
            ))}
          </AnimatePresence>
